...
JSON_DATA=$(jq -n \
  ...
  '{contents: [{parts: [{text: "Rephrase this git commit message in \($style) style using only the existing information. Keep it under 10 words. Original: \($msg). Respond ONLY with the rewritten message, no additional text."}]}]}')
